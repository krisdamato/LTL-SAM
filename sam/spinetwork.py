...
class SPINetwork:
    ...
    def measure_experimental_joint_distribution(self, duration, kernel_tau, resolution, sampling_timestep=10.0, smoothen=True):
        ...
        if smoothen:
            return self.get_distribution_from_smoothened_spikes(spikes, start_time, start_time + duration, kernel_tau, resolution, sampling_timestep)
        else:
            return self.get_distribution_from_spikes(spikes, start_time, start_time + duration, 1.5 * self.params['tau'], sampling_timestep, invalid_handling='none')
    ...
    def get_neuron_variable_mapping(self):
        ...
        for ym in self.__get_variables_ordered():
            i = int(ym[1:]) - 1
            variable_neuron_tuples = [self.get_variable_neurons(ym, x) for x in range(1, 1 + self.num_discrete_vals)]
            neurons = [n for tuple in variable_neuron_tuples for n in tuple]
            for n in neurons:
                neuron_dict[n] = ym
    ...
    def get_neuron_value_mapping(self):
        ...
        for ym in self.__get_variables_ordered():
            i = int(ym[1:]) - 1
            variable_neuron_tuples = [self.get_variable_neurons(ym, x) for x in range(1, 1 + self.num_discrete_vals)]
            for j in range(len(variable_neuron_tuples)):
                for n in variable_neuron_tuples[j]:
                    neuron_dict[n] = j + 1

        return neuron_dict
    
    
    def __convert_spikes_to_trains(self, spikes, start_time, end_time, timestep):
        """
        Converts NEST spikes into trains of spikes in the form of nest dictionaries: [var][value] => list
        """
        senders = spikes['senders']
        times = spikes['times']
        ...
        steps = np.arange(start_time, end_time + timestep, timestep)
        for t, n in zip(times, senders):
            var = var_map[n]
            value = value_map[n]
    ...
    def __get_alpha_kernel(self, timestep, tau):
        """
        Creates an alpha kernel with the specified timestep and decay constant (in ms).
        """
        # Find the point where the effect is insignificant (1/1000th)
        last_t = 912 # ms
        x = np.arange(0, last_t, timestep)
        y = x/(0.36787944117144233 * tau) * np.exp(-x / tau) # Rescale to 1.
        
        return y        

    
    def __smoothen_trains(self, trains, timestep, kernel_tau):
        """
        Creates a smoothing kernel and convolves the spike trains to turn them
        into firing rates.
        """
    ...
    def get_distribution_from_smoothened_spikes(self, spikes, start_time, end_time, kernel_tau, resolution, sampling_timestep):
        """
    ...
    def get_distribution_from_spikes(self, spikes, start_time, end_time, averaging_window = None, timestep=1.0, invalid_handling='none'):
        ...
        # Convert spike trains of each variable into a trace by smoothing.
